...
{    List<ContactDetails> listOfContacts; 
    ...
    @Ignore
    @Test
	public void contactDetailsWhenAdded_shouldReturnFirstNameOfTheMatchingPerson(){
		try {
			
			try {
				this.addressBookService.addContactToDB("Shubham", "Kumar", "CS-1-Hirapur","Friend", "Dhanbad", "Jharkhand",164512);
			} catch (SQLException e) {
				
				e.printStackTrace();
			}
			try {
				Assert.assertEquals(this.addressBookService.resultSetOpted.getString(1), "Shubham");
			} catch (SQLException e) {
